...
actions='["ai:read_agent_config", "ai:new_agent", "ai:build_prompt", "ai:call_llm", "sh:format"]'
/sh:flow --actions "$actions" --template "data:,###\n****************{{.prompt}}\n*************" --agent "ed" --message "correcto mine englise" --instruction "you are fantastic {{date}} agent: {{.agent}} adapter{{.adapter}}" --adapter "$adapter"
